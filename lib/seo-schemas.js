const BRAND_NAME = "Şahika Beauty"
const EXTENDED_BRAND_NAME = "Şahika Beauty - Sultanbeyli Güzellik Merkezi"

export const generateOrganizationSchema = () => ({
  "@context": "https://schema.org",
  "@type": "BeautySalon",
  name: BRAND_NAME,
  alternateName: EXTENDED_BRAND_NAME,
  description:
...
export const generateBreadcrumbSchema = (items) => ({
  ...
  itemListElement: items
    .filter(item => item && item.name && item.name.trim() !== '') // Filter out invalid items
    .map((item, index) => ({
      "@type": "ListItem",
      position: index + 1,
      name: item.name,
      item: item.url ? `https://sultanbeyliguzellikmerkezi.com.tr${item.url}` : undefined,
    })),
})
