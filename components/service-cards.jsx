...
const services = [
  ...
  },
  {
    ...
    buttonStyle:
      "border-blue-400 text-white hover:bg-blue-400 hover:text-white",
  },
]
        ...
        {services.map((service) => (
          ...
          >
            <CardContent className="p-6 md:p-8 text-center">
              <h3
                className={`text-xl md:text-2xl font-light italic ${service.scriptColor} mb-2`}
              >
                {service.scriptText}
              </h3>
              <h2
                className={`text-lg md:text-xl font-bold tracking-wider mb-4 md:mb-6 ${service.textColor}`}
              >
                {service.title}
              </h2>
              <p
              ...
              <Link href={service.link} target={service.id === 1 ? "_blank" : "_self"}>
                <Button
                  variant="outline"
                  className={`${service.buttonStyle} tracking-wider font-light bg-transparent text-xs md:text-sm px-4 md:px-6 py-2 transition-all duration-300`}
                >
                  {service.buttonText}
                </Button>
              </Link>
